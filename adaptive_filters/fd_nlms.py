...
class MDF:
    def __init__(self, fs: int, frame_size: int, filter_length: int) -> None:
        ...
        self.X = np.zeros((N, K, M + 1), dtype=np.complex128)
        self.Y = np.zeros((N, C), dtype=np.complex128)
        self.E = np.zeros((N, C), dtype=np.complex128)
        self.W = np.zeros((N, K, M, C), dtype=np.complex128)
        self.foreground = np.zeros((N, K, M, C), dtype=np.complex128)
            ...
            for chan in range(C):
                for speak in range(K):
                    for j in list(range(M)[::-1]):
                        self.PHI = np.concatenate([self.power_1, self.power_1[-2:0:-1]], axis=0) * self.prop[
                            j] * np.conj(self.X[:, speak, j + 1])[:, None] * self.E[:, chan][:, None]
...
if __name__ == "__main__":
    ...
    mic_out, sr = sf.read(inpath + '/mic_output.wav')
    ...
    if False:
        talk_len = len(mic)//4
        mic[talk_len:2*talk_len] += mic_out[:len(mic[talk_len:2*talk_len])]*10



    # 64 2048 for 8kHz.
    processor = MDF(sr, 64, 128)
    e, mic_est = processor.main_loop(ref, mic)
    mic_est = np.hstack([mic_est[1:] , mic_est[-1]])

    filtered_mic = mic - mic_est
   ...
   # Display
    ...
    axes[0, 0].plot(e, label='filtered mic  ')
    axes[0, 0].legend()
